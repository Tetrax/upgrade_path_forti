...
install -m 644 "$REPO_ROOT/deploy/fortios-recovery.service" /etc/systemd/system/fortios-recovery.service
install -m 644 "$REPO_ROOT/deploy/fortios-recovery.timer" /etc/systemd/system/fortios-recovery.timer
...
systemctl enable --now fortios-recovery.timer
systemctl restart fortios-recovery.timer
...
systemctl list-timers --no-pager fortios-catalog-refresh.timer fortios-recovery.timer fortios-cve-afternoon-refresh.timer
