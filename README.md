...
- `--cve-retry-delay-seconds N` (défaut 300) : avec ~50 requêtes HTTP séquentielles par jour vers le PSIRT, une advisory isolée qui timeout est courant et presque toujours transitoire (rate limiting, coupure brève côté Fortinet). Plutôt que de marquer tout de suite la source `cve-psirt` en avertissement, `fortios_watch.py` attend ce délai puis retente une seule fois les advisories restées en échec avant de calculer le statut de santé final — ce qui absorbe la quasi-totalité de ces échecs isolés sans jamais masquer une vraie panne PSIRT prolongée (si le second essai échoue aussi, l'avertissement reste affiché normalement). Mettre à `0` pour désactiver cette relance.
